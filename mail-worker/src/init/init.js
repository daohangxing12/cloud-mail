...
const dbInit = {
	...
	async v3_6DB(c) {
		const statements = [
			...
			`ALTER TABLE account ADD COLUMN bit_group_name TEXT NOT NULL DEFAULT '';`,
			...
			`CREATE INDEX IF NOT EXISTS idx_account_bit_group_name ON account(bit_group_name);`,
		...
		for (const statement of statements) {
			try {
				...
				console.warn(`skip v3_6 db statement: ${e.message}`);
			}
		}
	},
	...
	async v3_5DB(c) {
		const statements = [
			...
			`ALTER TABLE account ADD COLUMN creator_rewards_baseline_followers INTEGER NOT NULL DEFAULT 0;`,
			`ALTER TABLE account ADD COLUMN creator_rewards_baseline_views INTEGER NOT NULL DEFAULT 0;`,
	...
	async v3_4DB(c) {
		const statements = [
			...
			`ALTER TABLE account ADD COLUMN device_no TEXT NOT NULL DEFAULT '';`,
			`ALTER TABLE account ADD COLUMN device_updated_at TEXT NOT NULL DEFAULT '';`
		];
	...
	async intDB(c) {
		...
		await c.env.db.prepare(`
		  CREATE TABLE IF NOT EXISTS account (
			...
			bit_group_name TEXT NOT NULL DEFAULT '',
			...
			creator_rewards_baseline_followers INTEGER NOT NULL DEFAULT 0,
			creator_rewards_baseline_views INTEGER NOT NULL DEFAULT 0,
			device_no TEXT NOT NULL DEFAULT '',
			device_updated_at TEXT NOT NULL DEFAULT '',
