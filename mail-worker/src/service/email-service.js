...
const emailService = {
	...
	async blockSender(c, params, user) {
		const emailId = Number(params.emailId);
		if (!emailId) {
			throw new BizError('emailId required');
		}
		...
		if (!this.isAdmin(c, user) && Number(emailRow.userId) !== Number(user?.userId)) {
			throw new BizError(t('unauthorized'), 403);
		}

		const sender = this.normalizeSender(emailRow.sendEmail);
		if (!sender) {
			throw new BizError('sender email not found');
		}

		const settingRow = await settingService.query(c);
		const blackFrom = this.appendCsvUnique(settingRow.blackFrom, sender);
		await settingService.setBlacklist(c, {
			blackSubject: settingRow.blackSubject || '',
			blackContent: settingRow.blackContent || '',
			blackFrom
		});
		...
		const updateResult = await c.env.db.prepare(`
			UPDATE email
			SET is_spam = ?
			WHERE LOWER(send_email) = ?
				AND type = ?
				AND is_del = ?
		`).bind(emailConst.spam.SPAM, sender, emailConst.type.RECEIVE, isDel.NORMAL).run();

		return {
			sender,
			blackFrom,
			moved: Number(updateResult.meta?.changes || 0)
		};
	},
	...
	keywordFilter(keyword) {
		...
		return or(
			sql`LOWER(COALESCE(${email.toEmail}, '')) LIKE ${pattern}`,
			sql`LOWER(COALESCE(${email.sendEmail}, '')) LIKE ${pattern}`,
			sql`LOWER(COALESCE(${email.name}, '')) LIKE ${pattern}`,
			sql`LOWER(COALESCE(${email.subject}, '')) LIKE ${pattern}`,
			sql`LOWER(COALESCE(${email.text}, '')) LIKE ${pattern}`,
			sql`LOWER(COALESCE(${email.content}, '')) LIKE ${pattern}`
		);
	},
	...
	normalizeSender(value) {
		const sender = String(value || '').trim().toLowerCase();
		return verifyUtils.isEmail(sender) ? sender : '';
	},

	appendCsvUnique(current, value) {
