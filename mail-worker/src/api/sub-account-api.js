...
app.put('/subAccount/setCreatorStatus', async (c) => {
	const data = await subAccountService.setCreatorStatus(c, await c.req.json());
	return c.json(result.ok(data));
});

app.put('/subAccount/setDeviceNo', async (c) => {
	const data = await subAccountService.setDeviceNo(c, await c.req.json());
	return c.json(result.ok(data));
});
