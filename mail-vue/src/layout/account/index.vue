    ...
    <el-scrollbar class="scrollbar">
      ...
      >
        ...
        <div class="folder-list">
          ...
          <div class="folder-row" :class="folderActive(item.domain, 'spam')" @click="selectFolder(item, 'spam')">
...
watch(
    ...
    () => {
      if (domainAccounts.value.length === 0) return
      const exists = domainAccounts.value.some(item => item.domain === accountStore.currentDomain)
      if (!exists) {
        changeDomain(domainAccounts.value[0], false)
      }
      fetchDomainStats()
    },
    {immediate: true}
...
function selectFolder(item, name) {
  ...
  changeDomain(item, name === 'email' || name === 'spam')
