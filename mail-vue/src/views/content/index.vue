...
const handleBlockSender = () => {
  const mail = currentEmail()
  if (!mail.emailId || !mail.sendEmail || blockLoading.value) return

  ElMessageBox.confirm(`确认拉黑 ${mail.sendEmail}，并把历史同发件人邮件移入垃圾箱吗？`, '拉黑发件人', {
    confirmButtonText: t('confirm'),
    cancelButtonText: t('cancel'),
    type: 'warning'
  }).then(() => {
    blockLoading.value = true
    emailBlockSender(mail.emailId).then((data) => {
      ElMessage({
